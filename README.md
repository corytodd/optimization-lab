...
> **Licenses** -- Source code ([src/](src/), [include/](include/), [cmd/](cmd/),
> [test/](test/), [tools/](tools/)): [MIT](LICENSE).
> Documentation (this file): [CC BY-NC-ND 4.0](LICENSE-DOCS) -- you may share it
> with attribution, but may not republish or create derivative works.

Optimization is an iterative process that requires detailed knowledge of your domain, your target
system, and your toolchain; and a lot of patience. This repo teaches the _workflow_ through a
...
It is a Caesar cipher with shift 13. The key property that makes it self-inverse is that `((x + 13) % 26 + 13) % 26 == x`, so encoding and decoding are the same operation.
